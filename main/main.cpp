...
#if CONFIG_REMOTE_ONN
  if (!is_release_dbg) {
    // Show remap info for press events
    auto show_remap = [](uint32_t key) {
      auto *r = onn_lookup(key);
      if (r) {
        if (r->usb_report == USB_CONSUMER) printf(" -> Consumer(0x%04X)", r->output_code);
        else                               printf(" -> Keyboard(0x%02X)", r->output_code);
      } else {
        printf(" -> DROPPED");
      }
    };
    if (report_id == 1 && length >= 2) {
      show_remap(make_key(1, pData[0] | (pData[1] << 8)));
    } else if (report_id == 11) {
      for (int k = 2; k < 8 && k < (int)length; k++) {
        if (pData[k] != 0) show_remap(make_key(11, pData[k]));
      }
    }
  }
#endif
...
#if CONFIG_REMOTE_ONN
  // ---- ONN Remote: unified remap ----
  // Extract input codes, lookup in table, send press+release.
  // BLE release events (all zeros) are ignored.

  bool is_release = true;
  for (size_t i = 0; i < length; i++) { if (pData[i] != 0) { is_release = false; break; } }
  if (is_release) return;

  if (report_id == 1 && length >= 2) {
    // Consumer report: 16-bit usage in first 2 bytes (LE)
    uint32_t key = make_key(1, pData[0] | (pData[1] << 8));
    auto *r = onn_lookup(key);
    ...
    for (int k = 2; k < 8; k++) {
      if (pData[k] == 0) continue;
      uint32_t key = make_key(11, pData[k]);
      auto *r = onn_lookup(key);
      if (r) onn_send(r->usb_report, r->output_code);
    }
  }
...
#else
  // ---- Default (e.g. r_nz_201_180360): full pass-through ----
